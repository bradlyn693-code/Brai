...
function Dashboard() {
  ...
  const finishSwipe = (action: "pass" | "like" | "super") => { if (!current) return; setHistory((items) => [...items, { profile: current, action }]); x.set(0); y.set(0); controls.set({ x: 0, y: 0, rotate: 0, opacity: 1 }); setIndex((value) => value + 1); doMatch(current, action); };
